...
type EventsForDayProvider interface {
	EventsForDay(userId int, eventDate string) (*domain.ReqParameters, error)
}

func New(lg *slog.Logger, eventsForDayProv EventsForDayProvider) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// получение события
		resp, err := eventsForDayProv.EventsForDay(eventData.UserId, eventData.Date)
